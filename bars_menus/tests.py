...
class MenuTests(APITestCase):
    ...
    def setUpTestData(self):
        ...
        self.menu, _ = Menu.objects.get_or_create(user=self.user1, name='Gouter', bar=self.bar1)
    ...
    def test_create_menu_4(self):
        ...
        response = self.client.post('/menu/?bar=rugbyrouje', self.create_data, format='json')
        self.assertEqual(response.status_code, 404)

    def test_delete_menu_1(self):
        # Wrong permissions (wrong user)
        self.client.force_authenticate(user=self.user2)
    ...
    def test_update_menu_1(self):
        # Wrong permissions (wrong user)
        self.client.force_authenticate(user=self.user2)
        menu_id = self.client.get('/menu/').data[0]['id']
        response = self.client.put('/menu/%d/?bar=avironjone' % menu_id, self.create_data, format='json')
        self.assertEqual(response.status_code, 403)

    def test_update_menu_2(self):
        # Wrong permissions (wrong bar)
        self.client.force_authenticate(user=self.user3)
        menu_id = self.client.get('/menu/').data[0]['id']
        response = self.client.put('/menu/%d/?bar=avironjone' % menu_id, self.create_data, format='json')
    ...
    def test_update_menu_3(self):
        # Correct permissions
        self.client.force_authenticate(user=self.user1)
        menu_id = self.client.get('/menu/').data[0]['id']
        response = self.client.put('/menu/%d/?bar=avironjone' % menu_id, self.create_data, format='json')
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data["name"], 'Petit dejeuner')
        self.assertEqual(len(response.data["items"]), 2)
